...
$.ajax({
	...
	success: function (data) {
		...
		var CRpercentage = data.customer / 100 * 100;
		console.log(CRpercentage)
		$(".progress-bar-danger").css('width:' + CRpercentage + '%')
		$(".CRB").html(CRpercentage + "%");
		...
		var ODpercentage = data.deliveredorders / 100 * 1200;
		console.log(ODpercentage)
		$(".progress-bar-primary").css('width :' + ODpercentage + "%")
		$(".ODB").html(ODpercentage + "%");
...
$.ajax({
	...
	success: function (data) {
		...
		$(".TS").html(data.totalstock);
		$(".SD").html(data.totaldispatched_sea);
		$(".SDA").html(data.stockdamaged);
		// $(".SR").html(data.stockrebagged);
	},
	error: function (jqXHR, textStatus, errorThrown) {
		$.notify({
			message: "Error hitting endpoint check network connectivity"
